...
def post_process(input_color_image, input_depth_image, outputs):
    ...
    for i in indices:
        ...
        center = centers[i]
        # Draw bounding box.             
        cv2.rectangle(input_color_image, (left, top), (left + width, top + height), BLUE, 2*THICKNESS)          
        # Draw center 
    ...
    print(select_box_ind)
    if select_box_ind is not None:
        select_center = centers[select_box_ind]
        cv2.circle(input_color_image, select_center, 5, YELLOW, 2*THICKNESS)

        
    return input_color_image

def detect_result(input_color_image, input_depth_image, net):
    detections = pre_process(input_color_image,net)
    img_out = post_process(input_color_image, input_depth_image, detections)
    return img_out

modelWeights = os.path.dirname(__file__) + "/bin_picking.onnx"
